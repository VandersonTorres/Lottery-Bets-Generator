...
class Lottery:
    ...
    def play(self):                                                                 # This Method enables playing many times as user wants
        self.amount = int(input('Quantos jogos da MEGASENA você quer fazer?\n'))    # Amount of bets beeing played
        counter = 0
        while counter < self.amount:
            play = self.raffle_dozens()
            counter += 1
            if play == sena_2021:
                print (f'PARABÉNS! Você ganhou na {counter}ª jogada!')              # If user hits 'sena_2021', a congratulation message is displayed
                break
        print (f'Você jogou {counter} vezes!')                                      # Total amount of bets
        self.verify()
        
    def verify(self):                                                               # This Method verify if the program is going to keep running after a raffle
        question = input("Deseja continuar jogando?\n")
        if question in ["SIM","S","Sim",'sim','s']:
            _continue = Lottery(self.amount)
            _continue.play()
        else: print ("Você saiu da aplicação...")     
